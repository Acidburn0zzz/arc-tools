...
var Bump = {
   ...
   * - {String} target A build target: 'canary', 'dev', 'beta', 'beta-hotfix', 'stable',
   * 'stable-hotfix'
   * For `canary` release (daily build) the PATCH number will increase.
   * For `dev` release (weekly build) the BUILD number will increase.
   * For `beta` release  will increase the MAJOR and MINOR will be set to 0.
   ...
   * For `stable-hotfix` release the MINOR number will increase.
   */
  bump: (options) => {
