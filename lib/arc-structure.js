...
class ArcStructure {
  ...
  _publishElement(groupElement) {
    ...
    return this._exec('git add -A', dir)
    .then(() => this._exec('git commit -m "New: [CI] Added new elements."', dir))
    .then(() => this._exec('gulp --tasks', dir))
    .then((out) => {
      if (out.indexOf('└── release') === -1) {
        throw new Error('No `release` task!');
      }
      return this._exec('gulp release', dir);
    });
  }
