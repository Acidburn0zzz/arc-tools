...
class ArcStructure extends ArcBase {
  ...
  _readStructureInfo(item) {
    ...
    var file = this.current ? '' : item.element + '/';
    var main = item.main;
    var elementName;
    if (main instanceof Array) {
      if (main.indexOf(item.element + '.html') !== -1) {
        ...
        main = main[0]; // Is this OK?
        elementName = main.substr(0, main.indexOf('.'));
      }
    } else {
      elementName = main.substr(0, main.indexOf('.'));
    }
    ...
    .then((analyzer) => {
      ...
      if (!analyzer || !analyzer.elementsByTagName || !analyzer.elementsByTagName[elementName]) {
        if (this.verbose) {
          console.warn('  There were mo element documentation for main file', file);
        }
        return false;
      }
      let elm = analyzer.elementsByTagName[elementName];
      let tag = null;
      if (elm.jsdoc.tags) {
  ...
  _installGroupDeps(groupElement, dependency) {
    ...
    return this.exec(`bower install --silent -F --save ${dependency}`, dir);
  }
