 ...
 * 7) Publish package
 */
class ArcRelease extends ArcBase {
  ...
  commit() {
    ...
    .then(() => this.exec(`git commit -m "${this.commitMessage}"`));
  }

  push() {
    return this.exec('git push origin master')
    .then(() => this.exec('git push origin develop'));
  }

  tag() {
    return this.exec(`git tag -a ${this.version} -m "Created Tag for version: ${this.version}"`)
    .then(() => this.exec('git push origin master --tags'));
  }
  ...
  _ensureDirStructure() {
    ...
    var promises = structure.map((path) => this.ensurePath(path));
  ...
  _createPackage() {
    return new Promise((resolve, reject) => {
      ...
      this.ensurePath(this.distTarget)
      .then(() => {
  ...
  _applyBranding() {
    ...
    var copy = (file, i) => {
      return new Promise((resolve, reject) => {
        try {
          fs.writeFileSync(destIcons[i], fs.readFileSync(file));
          resolve();
        } catch (e) {
          reject(e);
        }
