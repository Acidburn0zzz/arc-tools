...
class ArcStage extends ArcBase {
  ...
  _bump() {
    ...
    .catch(() => {
      // no file...
      console.info('  No bower file available');
      return null;
    })
    .then((bower) => {
      if (bower) {
    ...
    .catch(() => {
      // no file...
      console.info('  No package file available');
      return null;
    })
    .then((data) => {
      if (data) {
        if (!newVer) {
          newVer = semver.inc(data.version, 'patch');
        }
        data.version = newVer;
  ...
  _changelog() {
    ...
    .catch(() => {
      // No changelog file. This is not crutial so do nothing.
      console.log('  No CHANGELOG.md file. Skipping.');
    });
  }
  ...
  _mergeMaster() {
    ...
    var cmd = 'git merge -m "[ci skip] Automated merge stage->master." ';
    cmd += '--no-ff stage -X theirs';
    return this.exec(cmd, this.componentDir)
    .then((result) => {
      if (this.verbose) {
