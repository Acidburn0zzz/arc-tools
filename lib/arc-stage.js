...
class ArcStage extends ArcBase {
  ...
  _mergeMaster() {
    ...
    return this.exec(`git merge --no-ff -m "[ci skip] Automated merge stage->master." stage`,
      this.componentDir)
    .then((result) => {
      if (result.indexOf('Already up-to-date')) {
        if (this.verbose) {
          console.log('Master up to date after merge, will not push changes.');
        }
      } else {
        if (this.verbose) {
