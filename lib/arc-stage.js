...
class ArcStage extends ArcBase {
  ...
  _ensureWorkingDir() {
    ...
    return this.ensurePath(this.dir);
  }

  _clearWorkingDir() {
    return this.exec(`cd .. && rm -rf -- ${this.dir}`, this.dir)
    .catch(() => {});
  }
  ...
  _clone() {
    ...
    try {
      fs.accessSync(this.workingDir, fs.F_OK);
      return this._pullLatest();
    } catch (e) {}

    return this.clone(this.ssh, {
      dir: this.dir
    });
  }
  ...
  _pullLatest() {
    if (this.verbose) {
      console.log('Pulling latest ' + this.component);
    }
    return this.exec(`git pull origin master`, this.dir)
    .then(() => this.exec(`git pull origin stage`, this.dir))
    .then(() => this.exec(`git checkout master`, this.dir));
  }
  ...
  _checkoutMaster() {
    ...
    return this.exec(`git checkout master`);
  }
  ...
  __readVersion() {
    ...
    return this.getBower(this.workingDir)
    .then((data) => {
      return data.version;
    });
  }
  ...
  _checkoutStage() {
    ...
    return this.exec(`git checkout stage`);
  }
  // Bump version number.
  _bump() {
    if (this.verbose) {
  ...
  _commitChanges() {
    ...
    return this.exec(`git commit -m "[CI] Automated commit after stage build."`);
  }
  ...
  _mergeMaster() {
    ...
    return this.exec(`git merge --no-ff -m "[CI] Automated merge stage->master"`);
  }
  ...
  _push(origin) {
    ...
    return this.exec(`git push origin ${origin}`);
  }
}
