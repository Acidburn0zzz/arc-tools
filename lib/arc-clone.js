  ...
  getStructure() {
    ...
    .then((repos) => {
      repos = repos.map((item) => {
        return {
          name: item.node.name,
          ssh: `git@github.com:${this.org}/${item.node.name}.git`,
          url: `https://github.com/${this.org}/${item.node.name}.git`
        };
      });
      // let res = [];
      // r.forEach((item) => {
  ...
  cloneNext() {
    ...
    .then((requirements) => {
      ...
      if (this.verbose) {
        ...
        console.log(`Bower required: ${requirements.requireBower}`);
        console.log(`Npm required: ${requirements.requireNode}`);
      }
      return this.deps(itemDir, requirements);
    })
    .then(() => {
