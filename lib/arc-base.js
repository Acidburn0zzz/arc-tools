...
class ArcBase {
  ...
  clone(url, opts) {
    ...
    opts = opts || {};
    var dir = opts.dir || './';
    var repoDir = opts.repoDir || path.join(dir, this.getRepoName(url));
    ...
    .then(() => this._getLastCommitSha(repoDir))
    .then((sha) => {
      if (!sha) {
        return null;
      }
      return this.exec(`git diff-tree --no-commit-id --name-only ${sha}`, repoDir);
    })
    .then((output) => {
      return this._readPackagedChangedUpdate(output);
    });
  }
  ...
  // Reads the repo name from the URL.
  getRepoName(url) {
