...
class ArcBase {
  ...
  deps(dirPath, requirements) {
    ...
    requirements.requireNode = requirements.requireNode || true;
    requirements.requireBower = requirements.requireBower || true;
    ...
    if (requirements.requireNode) {
      ...
      .then(() => {
        ...
        .then(() => this.exec('bower install --silent -F', dirPath));
